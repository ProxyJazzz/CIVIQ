...
export default async function LeaderboardPage() {
  ...
  // Gold, Silver, Bronze badges and shadows for podium
  const podiumOrder = [
    { 
      item: topThree[1], 
      place: 2, 
        ...
        <div className="space-y-8">
                ...
                >
                  <div className="relative">
                    <Avatar className="h-14 w-14 border-2 border-[#050608]">
                      ...
                      <AvatarFallback className="bg-muted text-foreground text-xs font-bold">{getInitials(name)}</AvatarFallback>
                    </Avatar>
                    <span className={`absolute -top-1.5 -right-1.5 flex h-5 w-5 items-center justify-center rounded-full border text-[9px] font-black ${badgeColor}`}>
                      {place}
                    </span>
                  </div>
          ...
          <div className="bg-[#0B0E13]/60 rounded-3xl border border-white/8 overflow-hidden shadow-2xl shadow-black/25">
            <div className="p-6 border-b border-white/5">
              ...
              <p className="text-[10px] text-muted-foreground mt-1">Points formula: Reports × 10 + Verifications × 5 + Votes × 2</p>
            </div>
              ...
              <Table>
                <TableHeader className="bg-white/2">
                  <TableRow className="border-b border-white/5 hover:bg-transparent">
                    <TableHead className="w-16 text-center text-[10px] font-black uppercase tracking-wider text-muted-foreground">Rank</TableHead>
                    <TableHead className="text-[10px] font-black uppercase tracking-wider text-muted-foreground">User</TableHead>
                    <TableHead className="text-center text-[10px] font-black uppercase tracking-wider text-muted-foreground">Reports Filed</TableHead>
                    <TableHead className="text-center text-[10px] font-black uppercase tracking-wider text-muted-foreground">Verifications</TableHead>
                    <TableHead className="text-center text-[10px] font-black uppercase tracking-wider text-muted-foreground">Votes Cast</TableHead>
                    <TableHead className="text-right pr-6 text-[10px] font-black uppercase tracking-wider text-muted-foreground">Score</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {users.map((item, index) => {
                    const name = item.full_name || "Citizen"
                    return (
                      <TableRow key={item.id} className="border-b border-white/5 hover:bg-white/5 transition-colors duration-200">
                        ...
                        <TableCell className="text-center text-xs font-mono text-white py-3.5">{item.reports_count}</TableCell>
                        <TableCell className="text-center text-xs font-mono text-white py-3.5">{item.verifications_count}</TableCell>
                        <TableCell className="text-center text-xs font-mono text-white py-3.5">{item.votes_count}</TableCell>
